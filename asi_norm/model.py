...
import asi_norm.input_dataset as geds
...
from asi_norm.interpolation import Interpolation as interpolation
...
class AttentionSpatialInterpolationModel:
    ...
    def __init__(self, id_dataset: str = None, num_nearest: int = None, early_stopping: bool = True,
        ...
        # general outputs
        if not os.path.exists(self.path + '/output_norm/hyperparameters/' + self.id_dataset):
            os.makedirs(self.path + '/output_norm/hyperparameters/' + self.id_dataset)
        if not os.path.exists(self.path + '/output_norm/images/architecture/' + self.id_dataset):
            os.makedirs(self.path + '/output_norm/images/architecture/' + self.id_dataset)
        if not os.path.exists(self.path + '/output_norm/images/nearest/' + self.id_dataset):
            os.makedirs(self.path + '/output_norm/images/nearest/' + self.id_dataset)
        if not os.path.exists(self.path + '/output_norm/models/' + self.id_dataset):
            os.makedirs(self.path + '/output_norm/models/' + self.id_dataset)
        if not os.path.exists(self.path + '/output_norm/result/' + self.id_dataset):
            os.makedirs(self.path + '/output_norm/result/' + self.id_dataset)
    ...
    def train(self, model, num_nearest_geo, num_nearest_eucli,
        ...
        filepath = self.path + '/output_norm/models/' + self.id_dataset + '/' + weights_locate
...
#                             validation_split=validation_split, verbose=1, callbacks=[ud.history, checkpoint])
    ...
    def predict_value(self, model, weights, num_nearest_geo, num_nearest_eucli, scale_log: bool = True,
        ...
        try:
            ...
            # load weights
            model.load_weights(self.path + '/output_norm/models/' + self.id_dataset + '/' + weights)
    ...
    def architecture(self, fitted_model, label: str = None):
        ...
        file_path_arc = self.path + '/output_norm/images/architecture/' + self.id_dataset + '/' + label + '.png'
    ...
    def output_layer(self, model, weight, layer, data, batch, file_name):
        ...
        # load weights
        model.load_weights(self.path + '/output_norm/models/' + self.id_dataset + '/' + weight)
        ...
        np.save(self.path + '/output_norm/result/' + self.id_dataset + '/'+file_name, predict)
